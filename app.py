...
import io  # <--- NEW IMPORT: Required for creating the Excel file in memory
...
# --- NEW FUNCTION: GENERATE TEMPLATE ---
def generate_excel_template():
    """Generates a sample Excel file in memory with the required structure."""
    
    # Define specific columns required by process_data
    columns = [
        'Year', 'Revenue', 'EBITDA', 'Net_Income', 'Depreciation', 'Interest_Expense',
    ...
    # Create Dummy Data for a sample company
    data = {
        'Year': [2022, 2023, 2024],
        'Revenue': [10000, 11000, 12100],
        'EBITDA': [2500, 2750, 3025],
        'Net_Income': [1800, 1980, 2178],
        'Depreciation': [200, 220, 240],
        'Interest_Expense': [50, 50, 50],
        'CapEx': [300, 330, 360],
        'Change_in_WC': [100, 110, 120],
        'Total_Debt': [500, 450, 400],
        'Cash_Equivalents': [1000, 1200, 1500],
        'Shares_Outstanding': [1000, 1000, 1000],
        'Avg_Price': [250, 275, 300],
        'Beta': [0.85, 0.85, 0.85],
        'Risk_Free_Rate': [0.07, 0.07, 0.07], # 7%
        'Market_Return': [0.12, 0.12, 0.12],   # 12%
        'Terminal_Growth': [0.05, 0.05, 0.05], # 5%
        'Projected_Growth': [0.10, 0.10, 0.10],# 10%
        'Tax_Rate': [0.25, 0.25, 0.25],        # 25%
        'Total_Equity': [5000, 5500, 6000],
        'Total_Assets': [6000, 6500, 7000],
        'Current_Assets': [2000, 2200, 2400],
        'Current_Liabilities': [1000, 1100, 1200]
    }
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, sheet_name='SAMPLE_CO', index=False)
        # Peer example
        df_peer = df.copy()
        df_peer['Revenue'] = df_peer['Revenue'] * 0.8 
        df_peer.to_excel(writer, sheet_name='PEER_CO', index=False)
...
def main():
    ...
    st.sidebar.download_button(
        ...
        data=excel_template,
        file_name="sentinel_valuation_template.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        help="Download a formatted Excel file with sample data."
    )
    st.sidebar.markdown("<div style='font-size: 11px; color: #95a5a6; margin-bottom: 10px;'>Use this template to structure your data.</div>", unsafe_allow_html=True)
    # -------------------------------------
    ...
    if uploaded_file:
        ...
        st.info("👋 Welcome to IFAVP. Please upload the 'fmcg_data_detailed.xlsx' file to begin, or download the template from the sidebar.")
