...
# --- 2. PREMIUM CORPORATE CSS (High Contrast) ---
st.markdown("""
<style>
    /* Global Typography */
    html, body, [class*="css"] {
    ...
    /* Metric Cards - WHITE on GREY Background */
    .metric-card {
...
def generate_pdf(ticker, target, upside, wacc, ke, tg, proj_df, latest_data):
    ...
    summary_text = (
        f"This report presents a Discounted Cash Flow (DCF) valuation for {ticker} based on the "
        f"Free Cash Flow to Firm (FCFF) approach. "
        f"The derived intrinsic value is INR {target:,.2f} per share, implies a potential upside of {upside:+.1f}% "
        f"against the current market price."
    )
    ...
    pdf.set_font("Arial", 'B', 12)
    pdf.cell(0, 10, "Cash Flow Projections (First 5 Years)", 0, 1)
...
def main():
    ...
    if not st.session_state.get('authenticated', False):
        ...
        with c2:
            st.markdown("""<div style='background-color: #ffffff; padding: 30px; border-radius: 10px; border: 1px solid #ddd; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05);'><h3>IFAVP Login</h3><p style='color: #666;'>Institutional Access Only</p></div>""", unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        # --- KPI CARDS (Styled with White Background) ---
        ...
        with t1:
            ...
            fig.update_layout(title="Enterprise Value Composition", height=350, plot_bgcolor='white', paper_bgcolor='white', font={'color': '#333'})
        ...
        with t3:
            ...
            with c_trend1:
                ...
                fig_rev.update_layout(plot_bgcolor='white', paper_bgcolor='white', font={'color': '#333'})
                st.plotly_chart(fig_rev, use_container_width=True)
                st.markdown('</div>', unsafe_allow_html=True)
            with c_trend2:
                st.markdown('<div class="content-card">', unsafe_allow_html=True)
                fig_prof = px.line(df, x='Year', y=['EBITDA', 'Net_Income'], title="Profitability", markers=True)
                fig_prof.update_layout(plot_bgcolor='white', paper_bgcolor='white', font={'color': '#333'})
        ...
        with t4:
            ...
            with p1:
                ...
                fig_pe.update_layout(plot_bgcolor='white', paper_bgcolor='white', showlegend=False, font={'color': '#333'})
                st.plotly_chart(fig_pe, use_container_width=True)
                st.markdown('</div>', unsafe_allow_html=True)
            with p2:
                st.markdown('<div class="content-card">', unsafe_allow_html=True)
                fig_roe = px.bar(peer_df, x=peer_df.index, y='ROE', title="ROE (%)", color=peer_df.index)
                fig_roe.update_layout(plot_bgcolor='white', paper_bgcolor='white', showlegend=False, font={'color': '#333'})
